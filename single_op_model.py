"""
Automatically generating tflite models containing single tflite operation

Parameters
----------
OPNAME : str, mandatory
--outdir : str, optional
"""
import argparse
import sys
...
def __generate_model(op_name, tf_func, input_tensors, outdir, dtype=None):
    tspecs = []
    dtype = input_tensors[0].dtype if dtype is None else dtype
    for tensor in input_tensors:
...
def parse_args():
    ...
    parser.add_argument(
        "--outdir", action="store", default="./models", help="Output directory"
    )
